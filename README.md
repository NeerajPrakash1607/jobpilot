...
Open **Company**, search by name, then choose **Add its careers page** if it is missing. Sign in and use **Add company** to save a careers link or connect a Greenhouse, Lever (global or EU), or Ashby board. Saved links and connected companies appear together in one searchable A–Z list. Unsupported pages have an **Open careers page** link, a status and a Change link action; they cannot filter jobs and are excluded from job counts and alerts. Account-saved links stay in the list after a reload, and older browser-local links appear in the same list unless their canonical feed is now connected. JobPilot verifies feeds before saving a durable connection. The Apps Script runner receives connected additions on the next daily checking cycle; no script update is required. Refreshing listings does not connect unsupported links. This free pilot permits 50 added boards and 20 additions/requests per account.
