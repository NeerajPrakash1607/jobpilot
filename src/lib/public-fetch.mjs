...
export async function getPublicPage(input,redirects=0,maxBytes=2_000_000,{method='GET',body,headers={},cacheTtl=0}={}){
  ...
  // Generic Workday connections may fetch only the public listings endpoint, never arbitrary tenant paths.
  const workdayHost=url.hostname.match(/^([a-z0-9-]+)\.wd\d+\.myworkdayjobs\.com$/);
  const workdayPath=url.pathname.match(/^\/wday\/cxs\/([-a-zA-Z0-9_]{1,80})\/([-a-zA-Z0-9_]{1,100})\/jobs$/);
  const workdayFeed=workdayPath&&(workdayHost?.[1]===workdayPath[1]||/^wd\d+\.myworkdaysite\.com$/.test(url.hostname));
  if(url.protocol!=='https:'||url.port||(!hosts.has(url.hostname)&&!workdayFeed))throw new AppError('Use the browser companion to import this careers page, or paste its job description. This site is not connected to online imports.',422);
