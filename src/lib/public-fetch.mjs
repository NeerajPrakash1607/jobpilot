...
export async function getPublicPage(input,redirects=0,maxBytes=2_000_000,{method='GET',body,headers={},cacheTtl=0}={}){
  ...
  // This Workday host is shared by employers; allow only Fidelity's verified public jobs endpoint.
  const fidelityFeed=url.hostname==='wd1.myworkdaysite.com'&&url.pathname==='/wday/cxs/fmr/FidelityCareers/jobs';
  if(url.protocol!=='https:'||url.port||(!hosts.has(url.hostname)&&!fidelityFeed))throw new AppError('Use the browser companion to import this careers page, or paste its job description. This site is not connected to online imports.',422);
