import {WATCH_SOURCES,EMPTY_PREFERENCES,PILOT_CAPACITY,readWatchPreferences,nextVacancyState,sourceHealth} from './watch-domain.mjs';
...
const asAccount=row=>row?{id:row.id,email:row.email,name:row.name,status:row.status,preferences:readWatchPreferences(JSON.parse(row.preferences)),createdAt:row.created_at}:null;
export function watchStore(db){
 ...
 return {
  ...
  async catalog(now){const rows=await all('SELECT * FROM watch_sources');const records=new Map(rows.map(r=>[r.id,r]));
   return (await sources()).map(source=>{const r=records.get(source.id);return {id:source.id,name:source.name,url:source.home,scope:source.scope||'Employer board',supported:source.type!=='website',health:sourceHealth(source,r,now),lastAttempt:r?.last_attempt||null,lastSuccess:r?.last_success||null,count:r?.count||0,skipped:r?.skipped||0,partial:!!r?.partial,note:r?.error||source.note||''};});
  },
  ...
  async saveSnapshot(source,token,snapshot,now){
   const owns=await stmt('SELECT id FROM watch_sources WHERE id=? AND lock_token=?',source.id,token).first();if(!owns)return;
   if(snapshot.kind==='failed'){await stmt("UPDATE watch_sources SET status='failed',error=?,locked_until=0,lock_token=NULL WHERE id=? AND lock_token=?",snapshot.error,source.id,token).run();return;}
   const previous=(await all('SELECT * FROM watch_vacancies WHERE source_id=?',source.id)).map(asJob);
   const next=nextVacancyState(previous,snapshot,now);
   ...
   writes.push(stmt("UPDATE watch_sources SET last_success=?,status='ok',partial=?,count=?,skipped=?,error=NULL,locked_until=0,lock_token=NULL WHERE id=? AND lock_token=?",now,Number(!snapshot.complete),snapshot.jobs.length,snapshot.skipped,source.id,token));
  ...
  async preferences(id,prefs){await stmt('UPDATE watch_accounts SET preferences=? WHERE id=?',JSON.stringify(prefs),id).run();return account(id);},
  async subscribe(id,now){
   await stmt("UPDATE watch_accounts SET status=CASE WHEN status='active' THEN 'active' WHEN (SELECT COUNT(*) FROM watch_accounts WHERE status='active')<? THEN 'active' ELSE 'waitlisted' END,opted_at=COALESCE(opted_at,?) WHERE id=?",PILOT_CAPACITY,now,id).run();return account(id);
  },
  async pause(id){await stmt("UPDATE watch_accounts SET status='paused' WHERE id=?",id).run();return account(id);},
  async unsubscribe(hash){const r=await stmt("UPDATE watch_accounts SET status='unsubscribed' WHERE unsubscribe_hash=?",hash).run();return r.meta.changes>0;},
  ...
  async finishDelivery(id,state,now){
   ...
   if(['failed','cancelled'].includes(state))writes.push(stmt('DELETE FROM watch_delivered_jobs WHERE delivery_id=?',id));
   await db.batch(writes);
  },
  ...
  async promote(now){await stmt("UPDATE watch_accounts SET status='active' WHERE id IN (SELECT id FROM watch_accounts WHERE status='waitlisted' ORDER BY opted_at,id LIMIT MAX(0,?-(SELECT COUNT(*) FROM watch_accounts WHERE status='active')))",PILOT_CAPACITY).run();},
