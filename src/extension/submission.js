...
export function inspectSubmission({expectedUrl,commit=false,customAnswersReviewed=false}={}) {
  ...
  const uploadGroup=el=>el.type==='file'?el.closest('[role=group][aria-labelledby]'):null;
  const uploadLabel=el=>(uploadGroup(el)?.getAttribute('aria-labelledby')||'').split(/\s+/).map(id=>document.getElementById(id)?.textContent||'').join(' ').trim();
  const label=el=>(uploadLabel(el)||[...(el.labels||[])].map(n=>n.textContent).join(' ')||el.getAttribute('aria-label')||(el.getAttribute('aria-labelledby')||'').split(/\s+/).map(id=>document.getElementById(id)?.textContent||'').join(' ').trim()||el.closest('.ashby-application-form-field-entry')?.querySelector('label')?.textContent||el.name||el.id||'Question').replace(/[_*]/g,' ').replace(/\s+/g,' ').trim();
  ...
  if(forms.length!==1){
    if(!forms.length){
      const embedded=[...document.querySelectorAll('iframe[src]')].filter(visible).flatMap(frame=>{
        try{
          const url=new URL(frame.src,location.href);
          const board=url.searchParams.get('for'),id=url.searchParams.get('token');
          if(url.protocol!=='https:'||url.username||url.password||url.port||!['job-boards.greenhouse.io','boards.greenhouse.io'].includes(url.hostname)||url.pathname!=='/embed/job_app'||!/^[-\w]+$/.test(board||'')||!/^\d+$/.test(id||''))return [];
          url.search='';url.hash='';url.searchParams.set('for',board);url.searchParams.set('token',id);
          return [url.href];
        }catch{return [];}
      });
      const urls=[...new Set(embedded)];
      if(urls.length===1)return {ready:false,embeddedApplicationUrl:urls[0],blockers:['This application is embedded from Greenhouse. Open the direct form below, then reopen JobPilot there to fill it.']};
    }
    return {ready:false,blockers:[forms.length?'More than one application form was found. Submit manually.':'No supported application form was found. If this is a job description, use the employer’s Apply button first.']};
  }
  ...
  for(const field of fields) {
    ...
    const required=field.required||field.getAttribute('aria-required')==='true'||uploadGroup(field)?.getAttribute('aria-required')==='true'||Boolean(entryLabel&&[...entryLabel.classList].some(name=>/^_required_/.test(name)));
