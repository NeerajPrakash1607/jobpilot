...
export function fillApplication(payload) {
  ...
  function describe(el){const uploadGroup=el.type==='file'?el.closest('[role=group][aria-labelledby]'):null;const uploadLabel=uploadGroup?(uploadGroup.getAttribute('aria-labelledby')||'').split(/\s+/).map(id=>document.getElementById(id)?.textContent||'').join(' ').trim():'';const label=uploadLabel||[...(el.labels||[])].map(node=>node.textContent).join(' ');const aria=el.getAttribute('aria-label')||(el.getAttribute('aria-labelledby')||'').split(/\s+/).map(id=>document.getElementById(id)?.textContent||'').join(' ').trim();const ashbyLabel=el.closest('.ashby-application-form-field-entry')?.querySelector('label')?.textContent;return (label||aria||ashbyLabel||el.name||el.id||el.placeholder||'').replace(/[_\-*]/g,' ').replace(/\s+/g,' ').trim();}
