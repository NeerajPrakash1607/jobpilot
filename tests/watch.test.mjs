...
test('Ireland matching excludes wrong Dublin and unknown details without guessing eligibility',()=>{
 ...
 assert.equal(matchWatchJobs([{...vacancy,title:'Support Engineer'}],prefs).length,0);
...
test('daily digest reserves jobs and uncertain sends do not repeat next day',async()=>{
 const {store,close}=database();try{
  ...
  await store.claimSource('stripe','lock',now,0);await store.saveSnapshot(WATCH_SOURCES.find(s=>s.id==='stripe'),'lock',{kind:'success',jobs:[vacancy],complete:true,skipped:0},now);
...
test('new connected boards survive reload and participate in scheduled checks, preferences and digests',async()=>{
 const {db,store,close}=database();try{
  ...
  let calls=0;const fetchPage=async()=>{calls++;return JSON.stringify({jobs:[job]});};
  ...
  const digest=await nextDigest(reloaded,'https://jobpilot.example',secret,now+86400000);assert.ok(digest.body.includes(job.jobUrl));
