...
export function parseFeed(source, body) {
  ...
  for (const raw of parsed.jobs) {
    try {
      ...
      if (!/^\d+$/.test(providerId)) throw new AppError('Invalid provider job ID.');
      const updated = source.type === 'greenhouse' ? raw.updated_at : raw.publication_date;
      jobs.push({ ...job, id:`${source.id}-${providerId}`, providerId, sourceId:source.id, source:source.name,
        ...
        dateLabel:source.type === 'greenhouse' ? 'Updated' : 'Published',
      });
    } catch { skipped++; }
  }
...
export function createDiscovery({ directory, sources=SOURCES, fetchPage=url=>getPublicPage(url,0,16_000_000), now=Date.now }={}) {
    ...
    const operation = (async () => {
      ...
      if (!cache) {
        try {
          ...
          if (saved.url === source.url && typeof saved.body === 'string' && Number.isFinite(saved.fetchedAt) && saved.fetchedAt <= now()) cache = { ...parseFeed(source,saved.body), fetchedAt:saved.fetchedAt };
        } catch { /* A missing or old cache can be replaced from the public feed. */ }
      }
      if (cache && now()-cache.fetchedAt < source.ttl) { memory.set(source.id,cache); return { ...cache, status:'cached' }; }
      if (cache?.retryAt > now()) return { ...cache, status:cache.jobs.length ? 'stale' : 'unavailable' };
      try {
        const body = await fetchPage(source.url); const parsed = parseFeed(source,body);
        cache = { ...parsed, fetchedAt:now() };
        memory.set(source.id,cache);
        await mkdir(directory,{recursive:true});
        const temporary = `${filename}.tmp`;
        await writeFile(temporary,JSON.stringify({url:source.url,fetchedAt:cache.fetchedAt,body}),{mode:0o600});
        await rename(temporary,filename);
        return { ...cache, status:'live' };
      } catch {
        const usable = cache && now()-cache.fetchedAt < 24*60*MINUTE;
        cache = { jobs:usable ? cache.jobs : [], skipped:usable ? cache.skipped : 0, fetchedAt:usable ? cache.fetchedAt : 0, retryAt:now()+5*MINUTE };
        memory.set(source.id,cache);
  ...
  return {
    async search(search,profile,savedJobs) {
      const feeds = await Promise.all(sources.map(readSource));
      const all = feeds.flatMap(feed=>feed.jobs);
      const results = rankJobs(all,search,profile,savedJobs);
      return { search, searchedAt:new Date(now()).toISOString(), scanned:all.length, total:results.length, jobs:results.slice(0,100),
        sources:sources.map((source,i)=>({name:source.name,url:source.home,status:feeds[i].status,count:feeds[i].jobs.length,skipped:feeds[i].skipped,
          checkedAt:feeds[i].fetchedAt ? new Date(feeds[i].fetchedAt).toISOString() : null,
          note:source.type==='remotive' ? 'Public feed is delayed by 24 hours; cached for 6 hours.' : 'Employer job board; cached for 30 minutes.'})),
      };
    },
    async resolve(id) {
      text(id,100,true);
      const source = sources.find(source=>id.startsWith(`${source.id}-`));
      ...
      if (source.type === 'greenhouse') {
        ...
        let fresh;
        try { fresh = JSON.parse(await fetchPage(`https://boards-api.greenhouse.io/v1/boards/${source.id}/jobs/${job.providerId}`)); }
